...
This script trains a YOLOv11 (nano) model for pothole detection.
It includes features to:
1. Load pre-trained YOLOv11 weights (yolo11n.pt) for Transfer Learning.
2. Configure all hyperparameters explicitly (learning rate, augmentations, optimizer).
3. Support training resume from a checkpoint.
4. Measure and log total training time.
5. Save the best and last checkpoints to 'models/yolov11/weights/'.
6. Export the finalized best model to ONNX format.

Usage:
    # Standard training (50 epochs):
    python src/train_yolov11.py --epochs 50 --batch 16
    
    # Fast test run (1 epoch, CPU training):
    python src/train_yolov11.py --epochs 1 --batch 8
    
    # Resume training:
    python src/train_yolov11.py --resume
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="YOLOv11 Pothole Detection Training Script")
    ...
    print_separator()
    print("          POTHOLE DETECTION PROJECT: YOLOv11 TRAINING")
    print_separator()
    
    # Define paths
    dataset_yaml = os.path.abspath("dataset/data.yaml")
    results_dir = os.path.abspath("models/yolov11/results")
    weights_dir = os.path.abspath("models/yolov11/weights")
    ...
    # If resuming, load the last checkpoint. Otherwise, load pre-trained nano weights.
    if args.resume:
        ...
        print("Initializing YOLOv11 (nano) model with pre-trained weights for Transfer Learning...")
        # Ultralytics will download 'yolo11n.pt' automatically if not present locally
    ...
    # 3. Train Model
    print("\nStarting training loop with explicit hyperparameters...")
    ...
    try:
        ...
        results = model.train(
            ...
            batch=args.batch,             # Batch size (images per step)
            ...
            # Training Settings
            patience=10,                  # Early stopping patience (stop if no val improvement for 10 epochs)
            ...
            # Folder Outputs
            project=results_dir,          # Output directory project folder
            name="train_run",             # Output subfolder name
            exist_ok=True,                # Overwrite existing folders
            
            # Data Augmentation Parameters (Explicitly Configured)
        ...
        # ONNX is an open representation format for ML models, optimized for faster deployment inference
        if args.export and os.path.exists(best_pt_dest):
            ...
            if os.path.exists(onnx_src) and os.path.abspath(onnx_src) != os.path.abspath(onnx_dest):
                ...
                # Sometimes export saves directly adjacent to input weights
        ...
        print("Proceed to Phase 4: YOLOv8 Training.")
        print_separator()
