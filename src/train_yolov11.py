...
def plot_custom_training_curves(csv_path, output_png_path):
    ...
    try:
        ...
        # Plot Loss Curves
        axes[0].plot(df[epoch_col], df[train_box], label='Train Box Loss', color='#ff7f0e', linestyle='-', linewidth=2)
        axes[0].plot(df[epoch_col], df[val_box], label='Val Box Loss', color='#ffbb78', linestyle='--', linewidth=2)
        axes[0].plot(df[epoch_col], df[train_cls], label='Train Class Loss', color='#1f77b4', linestyle='-', linewidth=2)
        axes[0].plot(df[epoch_col], df[val_cls], label='Val Class Loss', color='#aec7e8', linestyle='--', linewidth=2)
        ...
        axes[0].set_title("YOLOv11 Training and Validation Loss Curves", fontsize=12, fontweight='bold')
        ...
        axes[1].set_title("YOLOv11 Precision Progression (mAP)", fontsize=12, fontweight='bold')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="YOLOv11 Pothole Training Script")
    parser.add_argument("--epochs", type=int, default=50, help="Number of training epochs")
    parser.add_argument("--batch", type=int, default=16, help="Batch size")
    args = parser.parse_args()
    
    print_separator()
    print("          PHASE 4: YOLOv11 POTHOLE DETECTION TRAINING")
    print_separator()
    ...
    # Load the official pre-trained YOLOv11 nano model
    print("  [INFO] Loading YOLOv11 nano weights for transfer learning...")
    # NOTE: official name for YOLOv11 nano is 'yolo11n.pt'
    model = YOLO("yolo11n.pt")
    
    # Define hyperparameter block - IDENTICAL to YOLOv8 settings for fair comparison
    training_params = {
        'data': data_yaml,
        'epochs': args.epochs,
        'imgsz': 640,
    ...
    print_separator()
    print("  [INFO] Initiating YOLOv11 training loop...")
    print_separator()
    ...
    try:
        ...
        print(f"  [ERROR] An error occurred during YOLOv11 training: {e}")
        sys.exit(1)
    ...
    curve_output_path = os.path.join(results_dir, "yolov11_training_curves.png")
